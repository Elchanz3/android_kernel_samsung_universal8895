...
static ssize_t set_min_lock_dvfs(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	// Kill it from now without interferring Game LauncherAdd commentMore actions
	return count;
...
#ifdef CONFIG_MALI_DVFS
DEVICE_ATTR(dvfs, S_IRUGO, show_dvfs, NULL);
...
DEVICE_ATTR(dvfs_max_lock, S_IRUGO, show_max_lock_dvfs, NULL);
