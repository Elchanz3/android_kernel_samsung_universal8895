...
{
	struct cmucal_clk_fixed_rate *frate = to_clk_fixed_rate(clk);
	void __iomem *offset;
	unsigned int rate;
...
{
	struct cmucal_clk_fixed_factor *ffacor = to_clk_fixed_factor(clk);
...
{
	struct cmucal_pll *pll = to_clk_pll(clk);
			       ...
			       unsigned char *width)
{
	...
	field = cmucal_get_node(idx | SFR_ACCESS_TYPE);
	if (!field) {
	...
	*width = field->width;
	...
	reg = cmucal_get_node(field->sfr | SFR_TYPE);
	if (!reg) {
		pr_info("%s:failed idx:%x sfr:%x\n", __func__, idx, field->sfr);
		return 0;
	}

	block = cmucal_get_node(reg->block | SFR_BLOCK_TYPE);
	if (!reg || !block) {
...
int ra_init(void)
{
	...
	for (i = 0; i < size; i++) {
		block = cmucal_get_node(i | SFR_BLOCK_TYPE);
		if (block && block->pa)
