...
static __init int exynos_ufc_init(void)
{
	struct kobject *kobj;
	int ret;

	kobj = kobject_create_and_add("exynos_ufc", kernel_kobj);
	if (!kobj)
		return -ENOMEM;

	ret = sysfs_create_group(kobj, &exynos_ufc_attr_group);
	if (ret)
		kobject_put(kobj);
...
static __exit void exynos_ufc_exit(void)
{
	struct kobject *kobj;
	...
	sysfs_remove_group(kobj, &exynos_ufc_attr_group);
	kobject_put(kobj);
}
...
static struct kobj_attribute cpucl0_volt_table_attr = __ATTR(cpucl0_volt_table, 0644, show_cpucl0volt_table, store_cpucl0volt_table);
static struct kobj_attribute cpucl1_volt_table_attr = __ATTR(cpucl1_volt_table, 0644, show_cpucl1volt_table, store_cpucl1volt_table);
static struct kobj_attribute cpucl2_volt_table_attr = __ATTR(cpucl2_volt_table, 0644, show_cpucl2volt_table, store_cpucl2volt_table);

static struct attribute *exynos_ufc_attrs[] = {
       &cpucl0_volt_table_attr.attr,
       &cpucl1_volt_table_attr.attr,
       &cpucl2_volt_table_attr.attr,
       NULL,
};

static struct attribute_group exynos_ufc_attr_group = {
       .attrs = exynos_ufc_attrs,
};

int exynos_ufc_init(void)
{
       struct kobject *kobj;
       int ret;
